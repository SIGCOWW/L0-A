...
今回は@<img>{lcd}に示すタイプの電子棚札を入手し、表示内容の書き換えを試しました。
...
//footnote[ishida][イシダにも、920 MHz帯の無線を使うようカスタムしたと思われるPRICERの棚札がラインナップされています。]
...
一般的なテレビやエアコンのリモコンの場合、信号を38 kHzの矩形波で変調@<fn>{modulation}した上で赤外線LEDを発行させています。
ところが、棚札で使われる赤外線通信の搬送周波数は1245 kHz@<fn>{1245khz}です。
市販されている赤外線受信モジュールでは受信不可、さらに棚札を分解しても受光素子がそのままASICに繋がっており外部からは何もできません。
//footnote[modulation][受信側では38 kHzの信号しか受信しないフィルタをかましておくことで、自然光や熱による赤外線と区別でき誤作動を減らしています。]
//footnote[1245khz][イシダのカタログより。]

とはいえ、受信できる方法はあるはずです。
発光側として、棚札の赤外線通信に使われる波長である880nm@<fn>{880nm}を出す赤外線通信LEDを用意し、ひとまず1245 kHzでスイッチングさせました。
なんとかしてこれを受信しましょう。
...
普段レスバトルに励んでいる我々にとってアナログ回路は荷が重い、ということでまず試したのはアナログ回路を肩代わりしてくれそうな「LT1328」という赤外線受信ICの利用です。
ただしブレッドボードで試してみましたが思うように動きません、基板まで起こしてみましたが変わりませんでした。おわり。

次に試したのは、SDR@<fn>{sdr}機器です。
搬送周波数が1245 kHzなので、SDRの入力に受光素子を接続すれば電波として扱えるのではないか？と考えました。
結果、わずかに反応した気がしますが、ノイズと区別できないレベルです。おわり。
//footnote[sdr][「Software Defined Radio」。電波を受信したあとの復調処理、言わばアナログ回路に相当する部分をソフトウェアで行えます。]
...
GitHubで「PRICER AB」という文字列を検索したり、ググりまくったり結果分かったことですが、ブラジルのWebサイトにて「ISTT4」という棚札の管理ツールが公開されているようです。
若干出所が怪しいですが、トランシーバと棚札の間で通信できているか確認する処理が実装されていて解析に役立ちそうな気配がします。
...
==== 物理層
まず物理層から述べていきます。
送信側は@<bou>{ある信号}を1245 kHzで変調し、波長880nmのLEDを点灯させます。
ここはすでに述べた情報で、イシダやPRICERのカタログに記載されている部分でした。

@<bou>{ある信号}とは、PP4CというPRICER独自の方式によって変調された信号のことです。
PP4Cは4PPM（4値パルス位置変調、4 Pulse Position Modulation）の一種で、2ビット単位で@<img>{pp4c}に示す信号に変換されます。
なお、信号の最初はHIGHが40us出力され、その後にデータが続く感じです。
//image[pp4c][PP4Cの変調]
この2ビットは、もともと4つで1バイトを構成しています。
...
「ビット列」というのは184 bitsで構成されており、各ビットが液晶のセグメントに対応しているようです@<fn>{bits}。
...
//footnote[bits][ただしフルビット使うわけではありません。ここで用いた棚札よりセグメントの多い棚札用に多く確保しているのではと思われます。]
//footnote[haishi][この配置、また実はコマンドも当たりを付けてブルートフォースして解析しました…大変でした…。]
全セグメントを点灯させる場合、「@<tt>{84 PLID 37000000F0E52EFAE12BBFF88FFFB8CB2FFEE33FEEF28BFFF88FBB CRC}」というデータを送ることになるでしょう。
...
--------------------------------------------------------------------
> Index >  | 139 & 89  & 55 & 10 & 60 & 114& 110& 163&113 & 79 & 33 & 83 & 183
--------------------------------------------------------------------
> Segment <| 7g  & 8a  & 8b & 8c & 8d & 8e & 8f & 8g & p2 & y2 & 9a & 9b & 9c
--------------------------------------------------------------------
> Index >  | 133 & 138 & 88 & 54 & 9  & 59 & 159&109 & 30 & 47 & 156&106 & 56
--------------------------------------------------------------------
> Segment <| 9d  & 9e  & 9f & 9g &10a &10b &10c &10d &10e &10f &10g &11a & 11b
--------------------------------------------------------------------
> Index >  | 26  & 76  & 160& 126&179 &129 & 95 & 1  & 51 & 151& 101& 172& 122
--------------------------------------------------------------------
> Segment <| 11c & 11d & 11e& 11f& 11g& 12a& 12b& 12c& 12d& 12e& 12f& 12g& p3
--------------------------------------------------------------------
> Index >  | 72  & 18  & 68 & 152& 118& 148& 98 & 48 & 42 & 92& 176& 142 & 22
--------------------------------------------------------------------
> Segment <| 13a & 13b & 13c& 13d& 13e& 13f& 13g& 14a& 14b& 14c& 14d& 14e& 14f
--------------------------------------------------------------------
> Index >  | 155 & 105 & 71  & 25 & 75 & 175& 125& 178& 128& 94 & 0  & 50 & 150
--------------------------------------------------------------------
> Segment <| 14g & 15a & 15b & 15c& 15d& 15e& 15f& 15g& 16a& 16b& 16c& 16d& 16e
--------------------------------------------------------------------
> Index >  | 100 & 154 & 104 & 70 & 24 & 74 & 174& 124& 153& 119& 69 & 39 & 73
--------------------------------------------------------------------
> Segment <| 16f & 16g & p4  & p5 & p6 & y3 &    &    &    &    &    &    &
--------------------------------------------------------------------
> Index >  | 173 & 123 & 21  & 44 & 20 & 15 &    &    &    &    &    &    &
//}
...
1245 kHzで変調して…というのは割としんどいところですが、furrtek氏が送信用のソフトウェアを公開していました。
//emlist{
furrtek/PrecIR
https://github.com/furrtek/PrecIR
//}
AVRマイコンに対してPCからデータを送ると、PP4Cに変換してGPIOを操作してくれます。
電子ペーパータイプの電子棚札にかかれており、本稿で@<tt>{0x84}としているヘッダが@<tt>{0x85}になっているのでそこは直す必要がありますが、非常に簡単になります。
AVR用のプログラム@<fn>{avr}と回路図@<fn>{fet}、PC用のプログラムが掲載されていて初心者にも優しいですね。
//footnote[avr][AVRに詳しくなく、ヒューズビットをおかしな値に設定してしまいましたが…。外部クリスタルの10 MHzを8分周して（1245 kHzとほぼ同じ）1.25 MHzを生成しているので、外部クロック入力かつ分周比1/1の「10101000」を設定する必要があります。]
...
その様子を記録した動画もあります。
//emlist{
Change segment for ESL
https://www.youtube.com/watch?v=6bipAbZzJJ4
//}
//subfig[表示書き換えの様子]{
//image[all-segment][全点灯][scale=0.46]
//image[nikkori-segment][これには棚札もニッコリ][scale=0.49]
//}

なお、末尾の「100」は100回繰り返し送信する、という意味です。
というのも、データ長が長くなるにつれて受信が難しくなるようで、ここでは再送することによってデータの到達性を上げています。
しかしこれでもうまく受信されないことがあり、先ほどのコマンドを何度も実行するはめになることもあります。


== 液晶の制御
先ほど触れなかった棚札（大）についてですが、そもそも何故触れなかったのでしょう？
それは棚札（大）は電池が切れてしまったこと、そして一度電池が切れてしまうと赤外線による操作を受け付けなくなるためです。
というのも、furrtek氏によると棚札はRAMしか備えていないため電源断によりファームウェアが揮発するとのことで、打つ手がありません。
...
のちに中国・深圳の基板製造業者であるElecrowへ発注し、たまに同人即売会で展示しています。
//image[sch][基板]
...
本稿は、法令や公序良俗に反する行為を幇助するものではありません。
