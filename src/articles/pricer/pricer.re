...
どうやらブラジルの代理店が「ISTT4」という、電子棚札の管理ツールを公開しているようです。
トランシーバと電子棚札の間で通信できているか確認できる機能があり、役立ちそうな気配がします。
...
PP4は4PPM（4値パルス位置変調、4 Pulse Position Modulation）の一種で、2ビット単位で図todoに示す信号に変換されます。

この2ビットは、元々4つで1バイトを構成しています。
ただしエンディアンが独特で、図todoのようになります。

==== データリンク層
PP4Cの上で流れるデータについて述べます。

これらのデータは、図todoのような構成になっています。
...
ここでCommandを@<tt>{0x17}、Payloadを@<tt>{00AA00}、つまり16進数表記で「@<tt>{84 PLID 1700AA CRC}」というデータを電子棚札に送ると、ACKとして棚札の赤外線LEDが光ってくれます@<fn>{other}。
...
そして念願の表示変更ですが、Commandを@<tt>{0x37}、Payloadを「@<tt>{0x000000 + ビット列}」にすると実現できます。
「ビット列」というのは184bitsで構成されており、各ビットが液晶のセグメントに対応しているようです。
MSB（最上位ビット）のインデックスを0、LSB（最下位ビット）のインデックスを183とすると、棚札（小）の場合は図todoのような配置になります@<fn>{haishi}。
...
きちんと回路図も掲載されていて、初心者にも優しいですね@<fn>{avr}。
...
//imagew[assign][セグメント番号]
//tabooularw[assign][ピンアサイン]{
...
前文で「値札を書き換えて半額弁当をゲットしよう！」とか書いていますが、実際にやると捕まると思います。
神奈川県や兵庫県に住んでいなくても捕まる可能性があるので、やめたほうがいいですね。
おわり。
