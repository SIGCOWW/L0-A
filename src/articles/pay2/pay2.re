...
//footnote[contactless][非接触決済のことです。ブランドのサービス名は「Visa payWave」や「Master contactless」など。最近発行したクレジットカードやApple Payで利用できます。]
...
振込先     | 銀行口座                 & PayPay残高
//}
//footnote[paypay][PayPayを選んだのは半ば消去法です。LINEは就活で遺恨が…それと審査が厳しいようなので見送りました。メルペイは検討時点でサービスインしていなかったこと、そして催事利用について問い合わせると腰の重そうな返答があったため…Origamiは鳶一折紙さんより五河琴里さんのほうがタイプなのでちょっと…。]
