...
//tabooularw[suica][交通系電子マネー取り扱いのために申し込んだ記録]{
...
「交通系電子マネー決済」という文字列を見るだけで心ときめく状態でした。
...
前々から@<img>{terminal_plus}のCoineyターミナルPlusで「対応予定」とはなっていましたが、すでに加盟している店舗のTwitterで案内が来たという書き込みがあったのです。
...
//footnote[jcb][Coineyの規約には「クレジットカード会社等による加盟店審査」とあり具体的な社名は記載されていませんが、JCBが入っているので株式会社ジェーシービーなのでは…。]
...
個人間送金機能を利用したわけではなく、審査を受けてユーザスキャン@<fn>{userscan}（@<img>{yomitoru}）加盟店として登録しています。
...
振込先     | 銀行口座                 & PayPay残高
//}
//footnote[paypay][PayPayを選んだのは半ば消去法です。LINEは就活で遺恨が…それと審査が厳しいようなので見送りました。メルペイは検討時点でサービスインしていなかったことと催事利用の可否が疑問で…Origamiは鳶一折紙さんより五河琴里さんのほうがタイプなのでちょっと…。]
//footnote[userscan][「よみとる決済」「店舗提示型」「カスタマースキャン」などと呼ばれることもありますが、ここでは「ユーザスキャン」で統一します。]
...
//footnote[storescan][「みせる決済」「利用者提示型」「コード支払い」「マーチャントスキャン」などと呼ばれることもありますが、ここでは「ストアスキャン」で統一します。]
//footnote[upc][実際に「pCAT（ルミーズ株式会社）」や「あっと決済（株式会社ビジコム）」が挟まっています。]

本稿執筆時点では審査中ですが、もし可決となればPayPayなど@<fn>{docomo}の決済が、UnivaPay StoreAppのiOS/AndroidアプリやAPIから利用できます。
なお、今後しばらくしても当サークルでストアスキャンが利用できない場合は…察してください。
//footnote[docomo][d払い・WeChat Pay・Alipayです。後ろ2つに関してはすでに審査通過となっており、さらにWeChat PayはCoiney側でも利用できるのですが、中国国内で本人確認の済んだアカウントでなければ決済できないためここでは触れません。]
...
実際に繋ぐ場合はEthernetで接続し@<fn>{ethernet}、その間に外部機器の操作をREST APIか何かで叩けるようにした装置を挟み込むとiPad側のアプリケーションから操作しやすくなるんじゃないですかね。
...
これまでに存在しているSquare Readerとその比較をFCCや技適を基に探すと@<table>{square-reader}のとおりになりました。
//tabooular[square-reader][Square Reader]{
FCC ID      | 工事設計認証番号 | 最終更新日
===========>|==================|========
...
とはいえ、混雑した会場でBluetoothを使うのは非常に不安なので、USBで接続できるSquare Stand@<fn>{square-stand}を使うと幸せになれたかも知れませんね。
...
そこで当サークルでは、各アプリと連携して起動と金額入力を自動で行う仕組みをネイティブアプリとして実装しし@<fn>{implements}利用しています。
その構成図と処理の流れは@<img>{software}のとおりです。
//image[software][アプリケーション構成図][scale=0.4]
//footnote[implements][機能および画面は開発中のものです。]

ここでSquareとの連携には、「Point of Sale API」を用いています。
誰でも見れる仕様書@<fn>{pos-api}によると内部ではCustom URL Schemeが利用されており、アプリ起動と決済金額の通知、さらに決済後は元のアプリに戻ってくることが可能です。
...
さらに相当な力技で、Square Reader専用のiPhone/Android端末を用意しておき、その端末をスイッチコントロールやadbから操作ということも…いやこれは厳しさしか感じないですね。
...
Coineyが使っているMiura Systemsの端末は、セキュリティの問題が指摘されていたり@<fn>{blackhat}@<fn>{square-miura}、FCCにはLCDテストや画面書き換えの手順も載っていたりして少し試したい気もありますが、攻めたことはやめます。
...
//footnote[square-miura][「この件を受けてSquareはMiura Systems製リーダーを廃止した」と書かれているニュースサイトもあり、昔は使えた…？どんな申込をしたらそんなことに…？という驚きでいっぱいです。]
