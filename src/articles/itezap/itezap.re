...
ならびに2018年12月に発行した「L0 Vol.6」に掲載した「アイマスキャラの回帰モデルつくってみた（試作）」の記事をマージしたものになります。
それは2つの記事が同じデータ分析の分野であり、使用するデータもほぼ同一であるためです。
そのため2つの記事をマージして、1つの記事として再編集しました。

またこれらの記事の執筆から月日がたち、新しいアイドルたちが増えました。
なので再編集にあたって、新たな子たちのデータに加えて、追加実験を行いました。
...
例えば「ありすって12才グループでもスレンダーな方だよなぁ」とか「育ちゃんこれで10才か…」など様々だと思います。
