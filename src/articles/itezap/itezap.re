...
2016年の冬コミC91でSIGCOWWが処女作「COMIC L0 vol.1」を発行、約2年間でvol.6まで発行されました。
この2年間でx院（xは任意の文字列）をなんとか出所できた人、志願して懲役を延長した人など、環境が大きく変化しました。
ですが、新刊を一度も落とさなかったのは奇跡だと思います。

今回は私がvol.6までに書いた記事を再録・加筆修正したものをまとめた総集編となります。
具体的には、vol.1,4,5,6の内容をマージしたり加筆修正したりしています。
当時の記事をそのまま再録したものもあります。
vol.2,3については禁則事項です。
変更点や修正部分がある場合、章のはじめに詳細を記述しています。
...
回帰モデルによるデータ分析においては、これらデータセットを使用します。
もう1つは、各シリーズの12歳以下のキャラデータを集合とするデータセットです。
クラスタリングによるデータ分析においては、このデータセットを使用します。
...
この2名に共通点はありませんが、吸血鬼とその僕という位置付けでユニットが結成されています。
...
//image[charlist_milli][ミリオンライブ幼少組の一覧][scale=0.8]
...
次に特徴量から年齢を除いた5次元の特徴量を使ってクラスタリングを行います。

実際に特徴量から年齢を除外しクラスタリングを行います。
...
年少組を対象とした年齢別クラスタリングでは、それなりによく分類できていたのではないでしょうか。
ただしこの結果はアイマス年少組だけによる分類結果なので、データが増えることによってより詳しく発育度合いがわかると思います。
...
本記事は、2018年8月に発行した「L0 Vol.5」に掲載した「タンパク質で見るデレマス次世代ママ候補初級編」の再録となります。
...
また、私の研究分野であるバイオ・ケモインフォマティクスとアイドルマスターを多少強引に結びつけた記事を紹介しました。
特にバイオ・ケモインフォマティクスの記事は、知らない人がまだ多い分野だと思うので、なかなかスパイスの効いた記事になったのではと思います。
...
また、引き続きバイオ・ケモインフォとアイドルマスターの記事は書いていきたいと思っています。