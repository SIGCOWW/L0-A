...
ワイ「あ、ほんとだ！この線を通じてメイン基板と通信しているということだね。SDA (Serial Data)とSCL (Serial Clock)とくればI2C@<fn>{i2c}だ！」
...
//}
チノ「分かりました。『0b01000000 (1 << 6)』というボタンIDを指定したらどうなるのか、若干気になりますが置いておきます。ちなみに、途中で水タンクが空になった場合、操作パネルのLEDが光りますがどういった通信になりますか？ボタンは押していないのでIRQをつかむといった操作は発生しないと思いますが。」
ワイ「@<img>{i2c-flow}における最後の通信、『@<tt>{Write [メモリアドレス][値]}』だけ発生するよ。あるメモリの状態（@<table>{memory}）に応じてLEDを光らせていると思う。」
//tabooularw[memory][状態表（表ヘッダがメモリアドレス。空欄はDon't careや未確認箇所。）]{
...
//footnote[lpc1114][ARMアーキテクチャで動くマイコン。実施当時はメーカーのセールが行われており価格が安く、さらにmbedとして利用できて扱いやすかったため人気があった。]
...
チノ「大変ですね。マスターはACKが返らないことを不審に思わなかったんですか？」
ワイ「おそらくマスターはソフトウェアで実装していて、スレーブの状態を一切考慮していないんじゃないかな？供給されるクロックが微妙に変化したりするし。」
...
=== ワイ「Bluetoothのスニファリング」todoスニファリングという表現
...
チノ「依存するWiresharkのバージョンが(todo)だったりして少し古いですが、使い方は簡単です。」
...
チノ「そうなんですか？」
ワイ「著者がTwitterに書いているし@<fn>{denwa}、文章中のuuidがバリスタアイのそれなんだよね。」
//footnote[denwa][@<href>{https://twitter.com/r00tapple/status/837580793295732736}]
チノ「この本には、認証なしでコーヒーをいれられたことが書かれていますね。ちょっと本に紹介されているコマンドを実行してみます。」
ワイ「お願いします。」
//noindent
―― さらに数日後 ――
チノ「動かないですね。」
...
ワイ「GATT (Generic Attributes)@<fn>{gatt}を扱うためのツールだね。」
//footnote[gatt][todo]
